...
const Portal = () => {
  ...
  const handleMint = async (itemValue) => {
    await window.ethereum.request({ method: 'eth_requestAccounts'})
			.then(async result => {
        // set ethers provider and signature
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const signer = provider.getSigner();
        ethers.utils.getAddress("0x1a34b9BE6621470Cd08F09e71Ad896b59e1Dd492");
        const tx = await signer.sendTransaction({
          to: "0x1a34b9BE6621470Cd08F09e71Ad896b59e1Dd492",
          value: ethers.utils.parseEther(itemValue)
        });
			})
  }
                ...
                <Row>
                  <Colxx xxs="12">
                    <div className="container">
                        {
                        items.map((item)=>{
                          return <Item item={item} key={item.id} handleMint={()=>{handleMint(item.price)}}/>
                        })
                      }
                    </div>
                  </Colxx>
